...
/// 定义XZProgressHUDDelegate
public protocol XZProgressHUDDelegate {
    func hudWasHidden(_ hud: XZProgressHUD)
}

extension XZProgressHUDDelegate {
    func hudWasHidden(_ hud: XZProgressHUD) {}
}

/// XZProgressHUD
open class XZProgressHUD: UIView {
    ...
    public var progress: Float = 0.0 {
        didSet {
            if (oldValue != progress) {
                if let progressView = indicator as? XZProgressView {
    ...
    public var bezelView: XZProgressHUDBackgroundView?
    public var backgroundView: XZProgressHUDBackgroundView?
    ...
    public var delegate: XZProgressHUDDelegate?
    ...
    public class func show(addedToView view: UIView, animated: Bool) -> Self {
    ...
    public class func hudForView(_ view: UIView) -> XZProgressHUD? {
        let subviews = view.subviews.reversed()
        for subview in subviews {
            if (subview is XZProgressHUD) {
                return subview as? XZProgressHUD
            }
        }
        return nil
    }
    ...
    func setupViews() {
        ...
        backgroundView = XZProgressHUDBackgroundView(frame: self.bounds)
        ...
        bezelView = XZProgressHUDBackgroundView()
        ...
        button = XZProgressHUDRoundedButton()
    ...
    func updateIndicators() {
        ...
        case .determinateHorizontalBar:
            indicator?.removeFromSuperview()
            indicator = XZBarProgressView()
            bezelView?.addSubview(indicator!)
        case .determinate:
            if !(indicator is XZRoundProgressView) {
                // Update to determinante indicator
                indicator?.removeFromSuperview()
                indicator = XZRoundProgressView()
                bezelView?.addSubview(indicator!)
            }
        case .annularDeterminate:
            if !(indicator is XZAnnularProgressView) {
                // Update to annular determinate indicator
                indicator?.removeFromSuperview()
                indicator = XZAnnularProgressView()
                bezelView?.addSubview(indicator!)
            }
        case .customView:
    ...
    func updateViews(forColor color: UIColor) {
        ...
        } else if let barProgressView = indicator as? XZBarProgressView {
            barProgressView.progressColor = color
            barProgressView.lineColor = color
        } else if let circleProgressView = indicator as? XZCircleProcessView {
    ...
    public override func updateConstraints() {
        ...
        sideConstraints.append(
            contentsOf: NSLayoutConstraint.constraints(
                withVisualFormat: "|-(>=margin)-[bezel]-(>=margin)-|",
                options: .alignAllTop,
                metrics: metrics,
                views: ["bezel": bezelView!]
            )
        )
        sideConstraints.append(
            contentsOf: NSLayoutConstraint.constraints(
                withVisualFormat: "V:|-(>=margin)-[bezel]-(>=margin)-|",
                options: .alignAllTop,
                metrics: metrics,
                views: ["bezel": bezelView!]
            )
        ...
        for (index, view) in subviews.enumerated() {
            ...
            bezelConstraints?.append(
                contentsOf: NSLayoutConstraint.constraints(
                    withVisualFormat: "|-(>=margin)-[view]-(>=margin)-|",
                    options: .alignAllTop,
                    metrics: metrics,
                    views: ["view": view]
                )
            ...
            if (index > 0) {
                // Has previous
                let padding = NSLayoutConstraint(
                    item: view,
                    attribute: .top,
                    relatedBy: .equal,
    ...
    func setProgressDisplayLinkEnabled(_ enabled: Bool) {
        if (enabled && (progressObject != nil)) {
            if (progressObjectDisplayLink == nil) {
                self.progressObjectDisplayLink = CADisplayLink(
                    target: self,
...
public class XZProgressHUDBackgroundView: UIView {
...
class XZProgressHUDRoundedButton: UIButton {
    // MARK: Lifecycle
    override init(frame: CGRect) {
        super.init(frame: frame)
...
class XZBarProgressView: XZProgressView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        context?.addArc(
            tangent1End: CGPoint(x: rect.size.width - 2.0, y: 2),
            tangent2End: CGPoint(x: rect.size.width - 2, y: rect.size.height / 2),
            radius: radius
        )
        context?.addArc(
            tangent1End: CGPoint(x: rect.size.width - 2, y: rect.size.height - 2),
            tangent2End: CGPoint(x: rect.size.width - radius - 2, y: rect.size.height - 2),
            radius: radius
        )
        context?.addLine(to: CGPoint(x: radius + 2, y: rect.size.height - 2))
        context?.addArc(
            tangent1End: CGPoint(x: 2, y: rect.size.height - 2),
            tangent2End: CGPoint(x: 2, y: rect.size.height / 2),
            radius: radius
        )
        context?.fillPath()
        ...
        if (amount >= radius + 4.0 && amount <= (rect.size.width - radius - 4.0)) {
            ...
            context?.move(to: CGPoint(x: 4, y: rect.size.height / 2))
            context?.addArc(
                tangent1End: CGPoint(x: 4, y: rect.size.height - 4),
                tangent2End: CGPoint(x: radius + 4, y: rect.size.height - 4),
                radius: radius
            )
        ...
        // Progress in the right arc
            ...
            context?.addArc(
                center: CGPoint(x: rect.size.width - radius - 4, y: rect.size.height / 2),
                radius: radius,
...
class XZCircleProcessView: XZProgressView {  // base class of Round and Annular viw
...
class XZRoundProgressView: XZCircleProcessView {
    // MARK: Drawing
    override func draw(_ rect: CGRect) {
        let context = UIGraphicsGetCurrentContext()
...
class XZAnnularProgressView: XZCircleProcessView {
    // MARK: Drawing
    override func draw(_ rect: CGRect) {
        // Draw background
