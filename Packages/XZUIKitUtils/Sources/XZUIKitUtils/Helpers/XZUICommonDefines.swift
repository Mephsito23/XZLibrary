...
let IS_IPOD = XZUIHelper.isIPod
let IS_IPHONE = XZUIHelper.isIPhone
let IS_SIMULATOR = XZUIHelper.isSimulator
...
let IOS_VERSION_NUMBER = XZUIHelper.numbericOSVersion
...
/// 设备屏幕尺寸
/// iPhoneX
let IS_58INCH_SCREEN = XZUIHelper.is58InchScreen
/// iPhone6/7/8 Plus
let IS_55INCH_SCREEN = XZUIHelper.is55InchScreen
/// iPhone6/7/8
let IS_47INCH_SCREEN = XZUIHelper.is47InchScreen
/// iPhone5/5s/SE
let IS_40INCH_SCREEN = XZUIHelper.is40InchScreen
/// iPhone4/4s
let IS_35INCH_SCREEN = XZUIHelper.is35InchScreen
// iPhone4/4s/5/5s/SE
...
let ToolBarHeight: CGFloat =
    (IS_LANDSCAPE
        ? PreferredVarForUniversalDevicesIncludingIPhoneX(44, 44, 53, 32, 32, 32)
        : PreferredVarForUniversalDevicesIncludingIPhoneX(44, 44, 83, 44, 44, 44))

/// tabBar相关frame
let TabBarHeight: CGFloat =
    (IS_LANDSCAPE
        ? PreferredVarForUniversalDevicesIncludingIPhoneX(49, 49, 53, 32, 32, 32)
        : PreferredVarForUniversalDevicesIncludingIPhoneX(49, 49, 83, 49, 49, 49))

/// 保护 iPhoneX 安全区域的 insets
let IPhoneXSafeAreaInsets: UIEdgeInsets = XZUIHelper.safeAreaInsetsForIPhoneX
...
/// 获取一个像素
let PixelOne = XZUIHelper.pixelOne
...
func PreferredVarForDevices<T>(_ varFor55Inch: T, _ varFor47or58Inch: T, _ varFor40Inch: T, _ varFor35Inch: T) -> T {
    return PreferredVarForUniversalDevices(
        varFor55Inch,
        varFor55Inch,
        varFor47or58Inch,
        varFor40Inch,
...
/// 同上，加多一个iPad的参数
func PreferredVarForUniversalDevices<T>(
    _ varForPad: T,
    _ varFor55Inch: T,
    ...
    return PreferredVarForUniversalDevicesIncludingIPhoneX(
        varForPad,
        varFor55Inch,
        varFor47or58Inch,
        varFor47or58Inch,
...
// 同上，包含 iPhoneX
func PreferredVarForUniversalDevicesIncludingIPhoneX<T>(
    _ varForPad: T,
    _ varFor55Inch: T,
    _ varFor58Inch: T,
    _ varFor47Inch: T,
    ...
    let result =
        (IS_IPAD
            ...
            : (IS_35INCH_SCREEN
                ...
                : (IS_40INCH_SCREEN
                    ? varFor40Inch
                    : (IS_47INCH_SCREEN ? varFor47Inch : (IS_55INCH_SCREEN ? varFor55Inch : varFor58Inch)))))
    return result
}
...
let UIImageMakeWithFileAndSuffix: (String, String) -> UIImage? = {
    UIImage(contentsOfFile: "\(Bundle.main.resourcePath ?? "")/\($0).\($1)")
}
...
func ReplaceMethod(_ _class: AnyClass, _ _originSelector: Selector, _ _newSelector: Selector) {
    ...
    let isAddedMethod = class_addMethod(
        _class,
        _originSelector,
...
extension CGRect {
    ...
    var isInf: Bool {
        return self.origin.x.isInfinite || self.origin.y.isInfinite || self.size.width.isInfinite
            || self.size.height.isInfinite
    }
...
extension NSRange {
    func containing(innerRange: NSRange) -> Bool {
        if innerRange.location >= self.location
            && self.location + self.length >= innerRange.location + innerRange.length
        {
            return true
        }
        return false
